...
user during transaction creation and are useful to limit miners
resource usage and prevent spam.

\dnote{constant block reward is important because of carlsten2016instability}

In most of blockchains users pays a fee proportional to transaction size,
limiting minerw {\em network} utilization. A rational Bitcoin miner does not
...
\dnote{link to qtum + loi luu may be useful here to show that this problem is somehow discussed in literature}

Finally, transactions utilize miners {\em storage} resources.
...
/dnote{Do we need coinbase transaction here?}
