...
We propose to charge only for a resource which consumed at most, so we can say about state-oriented, network-oriented or computation-oriented transactions. We provide an evaluation of Ethereum usage data which shows that splitting towards
resource is possible in this cryptocurrency.

We propose a convenient way to charge for state memory consumption~(considering coin lifetime also). Our approach is convenient for users who often do not know for how long they would like to store their coins in the system, and also convenient for miners. The approach is called scheduled payments, as we propose to charge periodically for the bytes of memory consumed.
