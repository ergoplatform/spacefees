...
state load
%не очень понятно, что значит state load
...
% эта мысль уже раз 5 повторялась, может хватит уже;) ", but the state requured to validate a transaction (e.g. Bitcoin UTXO) has to reside in expensive random access memory.
...
it was an execution of some code can lead to significant disbalance for a rich enough
...
argument in Eq.~\eqref{eq:max} becomes dominant with time. Starting from the moment $sT$ when the transaction happens, the
...
The desired properties of the current state size are the following:
...
exceeding the max state size is still present, but hardly ever being used. For example,
...
We propose more convenient method of charging; we name it {\em scheduled payments}. In this scheme a user must set a special predefined script for a coin~(otherwise a transaction and also a block containing it are invalid), which contains a user-specific logic~(we call it a {\em regular script}) and a spending condition which allows anyone~(presumably, a miner) to create a transaction claiming this output, necessarily creating a coin with the same guarding condition and a value not less than original minus a state fee. These two parts~(regular script and a fee charging condition) are connected by using the $\lor$ conjecture. We assume that $\alpha$ and $\beta$ are fixed. 
We also assume that subsidized period $sT$ is to be stored along with the coin by each validating node. Then a guarding script for the coin would be like:
...
\end{split}
\end{align}
where $height$ is a height of a block which contains a spending transaction; $out.height$ is a height when the output was created; $out.height$ and $out.script$ contain value and spending script of the output, respectively; $tx.has\_output()$ checks whether a spending transaction has an output with conditions given as the predicate arguments, and $S_c$ is the value of $S(state)$ when the coin is
% or was???
created. As in the Section~\ref{sec:algorithm}, the $B$ constant is the output size.     
