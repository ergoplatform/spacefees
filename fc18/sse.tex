...
to known security issues. We propose the modification of a transaction fee scheme
...
fee and demonstrate consistency of the approach, analysing the statistics from
...
in this paper we have shown that it is reasonable to assign the fee to the
transaction in a different manner, namely depending on the type of the utilized resource: network, computation or storage.
The data from Ethereum blockchain shows that significant amount of transactions
in such a 3D space are
distributed along the coordnate axes, allowing one to
unambiguously classify them by consumed resource.
...
It is required to make the state size more predictable, but the implementation is
challenging since lifetime of the outputs is not known at the time when the transaction is
...
Besides limiting the size of the state, storage fee has valuable side effects.
...
Another important side effect of the storage fee is an additional block reward
for miners. Even when all coins are emitted and fixed block reward goes to zero,
storage fee will provide reward for miners independently on transactions included into block.
...
which is believed to solve the problem of uncontrollable state growth, has
