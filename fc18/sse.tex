...
We propose to add a new component to a transaction fee, which is based on
how much additional space will be needed for new objects created as a result of
transaction processing and for how long will they live in the state.
...
We show a possible implementation for state-related fee in a form of regular payments to miners.
...
While in most of cryptocurrencies transaction fees are addressed as solid objects,
in the current paper we have showed that it is reasonable to divide the fee into three
parts depending on type of the utilized resource: network, computation or storage.
The analysis of Ethereum blockchain showed that transactions in such a 3D space are
distributed close to one of the 3 axese, allowing us to
unambiguously classify transactions by their consumed resource.
...
This fee is required to make the state size more predictable, but its implementation is
...
Due to the structure of this fee, it provides a way to return coins with lost keys into circulation.
...
in connection with the prevention of the deflation, which may eventually occur in cryptocurrencies with fixed supply. Coin recorculation
was even implemented in some cryptocurrencies\cite{freicoin}.

Another important side effect of the storage fee is that it provides an additional block reward
for miners. Even when all coins are emitted and fixed block reward goes to zero,
storage fee will provide reward for miners, that does not depend on transactions included into block.
