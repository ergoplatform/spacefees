...
spam prevention.
%In spite of the problems and the possible solution outlined in the introduction,the essential element of the space-scarce economy 
%is the method of assigning the fee to every transaction in a sensible way.
...
The evolution of the blockchain networks has demonstrated the main resources being used. 
First and the most important so far, the storage capacity of the nodes is limited. In particular, it
...
executed by all the nodes in the network. The extreme and most
...
almost zero overhead in terms of validation state size, but creates the network load
needed for synchronization.

\knote{refer here to a fee as a security measure concepts in the intro?}

A transaction fee should incorporate all the three of components above. We propose to charge 
for a component which demands more resources. That is, storage-oriented transactions should be 
charged for storage consumption, the computation-oriented transactions should be charged for 
script execution, and all the others by the network load. This can be formalized as follows:
\begin{equation}
    \operatorname{Fee}(tx) = \max(\alpha \cdot N_b(tx), \beta \cdot N_c(tx),
    S(state) \cdot T[tx])\,.
    \label{eq:max}
\end{equation}
Here $\alpha$ and $\beta$ are the pricing coefficients, $N_b(tx)$ is the
number of bytes in the transaction, which defines the network load, $N_c(tx)$ is
the estimate of the computational cost of transaction, $S(state)$ is the cost of
...
$tx_1$ for free.} \knote{Initially it was $S(state) \cdot (\sum{B(coin_{created})*L(coin_{created})} - \sum{B(coin_{created})*L(coin_{spent})})$, so the fee could be negative, but newly created coins are living for finite time nevertheless} 

The obvious questions here are as follows. What are the guiding principles for 
choosing $\alpha$, $\beta$ and $S(\cdot)$? How to arrange the charging for state
space-time \knote{was this term introduced before?} consumption, if transaction contains no information on when the
outputs will be spent? There is also the question of estimating $N_c(tx)$, which is extremely hard \vk{or
...
the last question beyond the scope of the paper, and answer affirmatively for the first two question further in this paper.
...
shown in the Appendix~\ref{apx:statesize}, this does not fully solve the problem of limiting the
state size. What is being controlled in this case, that is the rate at which the data
is being submitted, but not the state size itself. One could also manually
define the maximal size of the state for the network. This solution, in turn,
...
\begin{figure} 
    \hfill 
    \begin{tikzpicture} 
        \draw[thick,-stealth] (0,0) -- (0,3) node [right]{$p(x)$}; 
...
state size at price $p$ is given by $y = p \cdot N(p)$. An example is provided at
...
\subsection{An example of a pricing curve}
...
faster than $1/x$ moving closer to the limiting size. \knote{what does the last sentence mean?} On the other side, there
...
intorduced as described below.
\begin{figure}
    \center
    \includestandalone[width=.5\textwidth]{figures/subsid}
