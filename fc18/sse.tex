...
\title{Coins With Data: When Computer Memory Becomes a Scarce Resource}
...
\begin{itemize}
  \item{} all the fees for a block are going to just a miner like in Bitcoin.
      ...
      they are out of scope of the paper. A notable difference is that if a single
      is taking all the rewards, she can include her own transactions in it for free. 
\end{itemize}
...
As stated in the intro, a transaction fee is an anti-overload tool. Thus a transaction fee should 
incorporate all the three of components above. We propose to charge for a component which processing demands more resources. That is,  storage-oriented transactions should be charged for storage consumption, the computation-oriented transactions should be charged
for script execution, and all the others by the network load. The last sentence can be formalized in its simplest form as follows:
\begin{equation}
    \operatorname{Fee}(tx) = \max(\alpha \cdot N_b(tx), \beta \cdot N_c(tx),
    S(state,tx) \cdot T[tx])\,.
    \label{eq:max}
\end{equation}
...
the storage of outputs in the state for the unit of time, $T([tx])$ is the time which
...
$tx_1$ for free.} \knote{Initially it was $S(state) \cdot (\sum{B(coin_{created})*L(coin_{created})} - \sum{B(coin_{created})*L(coin_{spent})})$, so the fee could be negative, but newly created coins are living for finite time nevertheless} The obvious problems here are: What are the guiding principles
...
\subsection{Choice of the relative values of $\alpha$, $\beta$, $S(state,tx)$}
...
numbers: $N_b(tx)$, $N_c(tx)$, $N_s(tx) = S(tx)T[tx]$. Eq.~\eqref{eq:max} provides a
...
\subsection{Choice of $S(state,tx)$}

The simplest way of assigning the value of $S(state,tx)$ is by making it
...
state $p(x)$ varies with the current state load $x$. The values of $S(state,tx)$
and $p(x)$ in the continual limit are connected via
\begin{equation}
    S(state,tx)=\int_x^{x+\Delta(tx)}p(y)\,dy\,,
    \label{eq:Sp}
\end{equation}
...
In this section we propose a concrete way to charge for space consumed. There are few possible options. An user may, for example, to specify lifetime for a coin during its creation and pay in advance, but it is not very convenient for her. Another option is to charge when coins are being spent, or allow to spend a coin when its value is overweighted by space-time fee. As a drawback, if coin is associated with a big amount of tokens, it could live for very long. We propose more convenient method of charging, namely scheduled payments. 

\knote{``We assume that $\alpha$ and $\beta$ are fixed''}

In this scheme, \knote{How can a user set $sT$?}

If an user do not move the coins before this, then anyone~(presumably, a miner) can create a transaction claiming this output, returning all the coins back to the owner except of a space-time fee ${K \cdot B \cdot T}$. A spending script for the output would be like:

\begin{align}
\begin{split}
&(regular\_script) \lor \\
&(Height > (out.height + sT + T) \land (out.value \le K \cdot B \cdot T \lor \\  
&\qquad tx.has\_output(value = out.value - K \cdot B \cdot T, script = out.script)))
\end{split}
\end{align}

In this model $T$ is another protocol parameter which may be fixed by a protocol design or changed via miners voting like \textit{K}.
