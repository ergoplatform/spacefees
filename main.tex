...
\begin{equation}
r_{mode} = {R_{prepaid} \over R_{postpaid}} = {V_{output} \over  K \cdot B \cdot L_{max}} \approx {10^7 \over L_{max}}
\end{equation}
...
Since there are $N_{utxo}$ in a system and each of them contains $V_{output}$ coins, we can estimate mean number of output spendings per block incentivized by \esse{} model:

\begin{equation}
N_{incentivized} \approx {N_{utxo} \over L_{max-mean}} % = {N_{utxo} \cdot K \cdot B \over f_p}
\end{equation}

where $L_{max-mean}$ is mean lifetime of an output.
...
\label{minerrew}
Rewarding miners is very important for viability of a cryptocurrency economy. 
%- in original paper \cite{Nakamoto2008} it's assumed, that miners are incentivized to fair behaviour, while recent researches showed, that it is not always correct \cite{carlsten2016instability, eyal2014}. 
In addition to usual propagation fee, \esse{} provides two new sources for miner income: a space-time fee and a coin recirculation reward.
...
where $C$ is non-negative constant. A space-time reward depends on state size and space-time price $K$ only:
...
\begin{equation}
M_{recirculation} = {r_{mode} \cdot  N_{lost} \cdot K \cdot B}
\end{equation}
...
\begin{equation}
\label{eq:reward}
M_{reward} \approx M_{pg} + (K \cdot B) \cdot ((1 + r_flow) \cdot N_{utxo} + r_{mode} \cdot N_{lost})
\end{equation}
