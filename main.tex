...
\begin{equation}
R_{reward} = fee_{propogation} + fee_{space-time} + R_{recirculation}
\end{equation}

In section \ref{sec:flow} we have already noticed, \esse{} users have an intencive to actively use their coins and thus propogation fee is propotional to space-time fee.

Space-time fee is the same for all space-time fee models:

\begin{equation}
fee_{flow} = fee_{recirculation} \cdot N_{transactions} = K \cdot N_{UTXO} \cdot BS
\end{equation}

\dnote{fee_{space_time} = \sum_{i=1}^{N_{UTXO}} K_i \cdot BS_i \approx  K \cdot N_{UTXO} \cdot BS}
