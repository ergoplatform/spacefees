...
\textbf{A: } We assume that miners will behave rationally, so they do not want to kill the business by setting $K$ high. We propose to have a rule for changing $K$ slowly, as block gas limit in Ethereum~(say, a miner can increase or decrease $K$ by 0.1\% per block) so $L_{max}$ could be estimated before the transaction being made with a good precision.
