...
\begin{equation}
fee_{prepaid} = K \cdot (\sum_i{(B_i \cdot (L_i - Height))} - \sum_j{(B_j \cdot (L_j - Height)}))
\end{equation}
...
\begin{equation}
fee_{postpaid} = \sum_i{(K_i \cdot B_i \cdot (HS_i - HC_i))}
\end{equation}
...
\begin{equation}
M_{flow}={N_{UTXO} \cdot V_{output} \over L_{mean} \cdot N_{coins}}=K \cdot {N_{UTXO} \cdot V_{output} \cdot B \over fee_{propagation} \cdot N_{coins}} = K \cdot {B \over fee_{propagation}}
\end{equation}
