...
\begin{equation}
N_{moved-outputs} \approx {N_{UTXO} \over L_{mean}} = {N_{UTXO} \cdot K \cdot B \over f_p}
\end{equation}
...
For regular outputs of 36 Bytes space-time fee part will exceed propagation fee after 5555 blocks or 38 days. 
...
\begin{equation}
\label{eq:reward}
M_{reward} \approx M_{pg} + (K \cdot B) \cdot ((1 + C) \cdot N_{UTXO} + K_{model} \cdot N_{lost})
\end{equation}
