...
Recirculation reward was calculated in previous section and depends on space-time fee model. However it can be generalized as with $K_{model}$ with is equals to 1 for \textit{Leaking balance} and \textit{Postpaid} models, and $K_{model} > 1$ for \textit{Prepaid} model:
...
\begin{equation}
\label{eq:reward}
R_{reward} = R_{pg} + R_{space-time} \approx R_{pg} + (K \cdot BS) \cdot ((1 + M) \cdot N_{UTXO} + K_{model} \cdot N_{lost})
\end{equation}
...
that allows to estimate such $K$, when additional \esse{} reward will exceed common propagation reward $R_{pg}$ based on current Bitcoin statistics

\begin{equation}
K_{postpaid} = {R_{pg} \over BS \cdot N_{UTXO} \cdot (1 + M) \approx 10^(-9)} ({BTC / (Byte \cdot Block)})
\end{equation}

that is naturally coincide to estimations from section \ref{eq:ltmean} while both estimations are made from assumption, that propagation fee and space-time fee are equal for end users. Remarkable that space-time reward does not depend on network throughput and can be easily predicted. Since it will be accrued to generator of block with concrete height, it may be considered as a constant block reward fixing problem of blockchain instability without block reward \cite{carlsten2016instability}. Value of this reward may be controlled by $K$ parameter making mining more stable and thus network more secure.
