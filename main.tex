...
\subsection{Pre-payed outputs}

First model we are going to analyze is pre-payed outputs. In it each output represents a box of fixed size \textit{BS} in a \state{} and pre-payed until explicitly known period of time \textit{LT}. After \textit{LT} anyone ~(presumably miner, generated first block after that) will be able spend this overdue output. Thus spending script for all outoputs will be a combination of regular spending scrypt and loan period \textit{LT}:

$(Height > LT) \lor (regular\_scrypt)$

where \textit{Height} is current blockchain height and \textit{regular\_scrypt} is user-defined output spending script. This lifetime part of a scrypt is a part of consensus, and space-time transaction fee is payed at the time of output creation. :

$fee_{pre\-payed} = K \cdot (\sum_i{(BS_i \cdot (LT_i - Height))} - \sum_j{(BS_j \cdot (LT_j - Height)}))$

\subsection{Post-payed outputs}

In the second model, user should pay fee for taken scace-time in \state{} when he spends output. When output is spended it's lifetime is known, so resulting transaction fee can be calculated:

$fee_{post\-payed} = \sum_i{(K_i \cdot BS_i \cdot (HS_i - HC_i))}$

where \textit{HS_i} and \textit{HC_i} are heights, where i-th output was created and spended respectively. Note, that each output has it's own space-time \textit{K_i}, correspondind to \textit{K} value at it's creation time. Here we can note, that if output value is \textit{V}, then height \textit{H_{max}} exists, where all the transaction value will be taken by space-time fee.
...
$(Height > H_{max}) \lor (regular\_scrypt)$

%\subsection{Leacking balance}

%\dnote{По сути то же, что и Post-payed outputs? Transaction creates output with the same script with a value decreased by \textit{LB} Типа "после блока N потратить может кто угодно, но с учетом того, что 90\% вернется исходному владельцу"}
...
In this section we are going to provide comprehensive analysis of \esse model. Besides of it's main goal to control state size,
