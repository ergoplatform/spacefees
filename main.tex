...
$fee = max(fee_{validation\_cost} + fee_{propogation\_cost} + fee_{space-time\_fee}, 0)$ \dnote{why can't total fee be negative?}

Validation and transaction fees are out of scope of this paper and assumed to be independent from space-time fee. We also assume, that are positive and space-time fee impact to total fee may range from negliable to dominant.

space-time\_fee
%TODO как обеспечивать время жизни транзакций

$fee_{space-time\_fee} = K \cdot (\sum{(new\_output\_size \cdot lifetime - S_s)} - \sum{(old\_output\_size \cdot remaining\_lifetime - S_s)})$
