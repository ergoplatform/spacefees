...
As we have already mentioned above, in our model transaction fee consists of 3 parts: \textit{validation cost}, \textit{propogation cost} and \textit{state space-time difference cost}. Validation and propogation costs are strictly positive, while transaction may decrease state size and space-time fee may be nagative, so full transaction fee formula is as follows:

$fee = max(fee_{validation} + fee_{propogation} + fee_{space\_time}, 0)$ \dnote{why can't total fee be negative?}

Validation and transaction fees are out of scope of this paper and assumed to be independent from space-time fee. We also assume, that are positive and space-time fee impact to total fee may range from negliable to dominant. Space-time fee component dependency from size of inputs, outputs and their time to live may vary depending on the cryptocurrency purpose, in this paper we analize "natural" dependency, where user pays for multiplication of the time output data should be kept in \state{} to it's size. Also each transactoin delete some data from state and can fill this freed space-time for free and resulting space-time fee can be calculated.

$fee_{space\_time\_unsubsidized} = K \cdot (\sum{new\_output\_size \cdot lifetime)} - \sum{old\_output\_size \cdot remaining\_lifetime})$

where \textit{K} is a space-time price in fee tokens. This price depends on hardware price for miners and token exchange rate, so \textit{K} is not a static value. We assume, that miners can change this pricing via mechanism like ``gas'' in Ethereum and thus control cost of blockchain support.

To ensure output maximum lifetime, users should be forced to spend their outputs. For this purposes we propose rent-a-box model of outputs, where each output represents a box of fixed size in a \state{} and pre-pay it for some period of time. After this time box will be opened and anyone ~(presumably miner, generated first block after that) can spend this overdue output. Outoput lifetime should be explicitly included into it's spending script, that will be combination of regular spending scrypt and envirnoment, like blockchain height, for example

$(Height > 500) \lor (regular\_scrypt)$
...
%\esse{} provide an incentive to move your coins, \textbf{preventing economy stagnation}
%\minimal value in a box?
