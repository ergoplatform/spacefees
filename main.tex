...
\dnote{UTXO size = 46M}
\dnote{bitcoins in circulation = 16M}
\dnote{mean output size = 0.35}
\dnote{1 to 1 transaction fee = 0.0002 BTC}

\begin{itemize}
  \item number of unspent transaction outputs: $N_{UTXO}=46000000$
  \item number of coins in circullation: $N_{coins}=16000000$
  \item mean number of coins in one output: $V_{output}=0.35$
  \item simple payment transaction fee: $fee_{propogation}=0.0002$
\end{itemize}
...
To estimate money flow let's assume, that user will move his money when space-time fee will start dominanting over other transactional costs an therefore creation of transaction will be almost free for him. \dnote{}
