...
\subsection{Pre-paid outputs}

First model we are going to analyze is pre-paid outputs. In it each output represents a box of fixed size \textit{BS} in a \state{} and pre-paid until explicitly known period of time \textit{LT}. After \textit{LT} anyone ~(presumably miner, generated first block after that) will be able spend this overdue output. Thus spending script for all outoputs will be a combination of regular spending scrypt and loan period \textit{LT}:

$(Height > LT) \lor (regular\_scrypt)$

where \textit{Height} is current blockchain height and \textit{regular\_scrypt} is user-defined output spending script. This lifetime part of a scrypt is a part of consensus, and space-time transaction fee is paid at the time of output creation. :

$fee_{pre\-paid} = K \cdot (\sum_i{(BS_i \cdot (LT_i - Height))} - \sum_j{(BS_j \cdot (LT_j - Height)}))$

\subsection{Post-paid outputs}
...
$fee_{post\-paid} = \sum_i{(K_i \cdot BS_i \cdot (HS_i - HC_i))}$
