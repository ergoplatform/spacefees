...
The limited lifetime of a state element also leads to lost coins being taken back into circulation~(by miners). Another \dnote{2 sentence with the same meaning with Another bond} interesting effect is that the proposed fee regime is promoting money circulation in the blockchain economy.
...
\begin{itemize}
  ...
  \item{} For simplicity, we assume that a block is of a finite size but all the transactions a miner has at a given moment of time could be packed into it, if otherwise is not stated explicitly. \dnote{May be: For simplicity, we assume that a block is of a finite size and miner collect such transactions that maximize block reward}
...
After $L_{max}$ the output is spendable by anyone~(presumably, miner). \dnote{This was written few sentence ago: "After L max anyone (presumably a miner generating first block after that) is able to spend the overdue output"}
