...
\begin{abstract}
In this paper we study space-scarce economy in massively replicated open blockchain systems, such as Bitcoin. In these systems memory to hold a balance sheet needed to validate transactions becomes the most scarce resource eventually. Only technical solutions, not economic, have been proposed to tackle this problem to the moment. Uncontrolled state size growth could lead to spam attacks. The issue becomes more problematic for blockchain systems used to store data~(votes, certificates, logs etc.). As a solution, we propose to add a new component to a transaction fee scheme, which is a fee with a mandatory minimum based on how much additional space will be needed for new objects created in result of transaction processing, and fow how long they will live in system state. We provide three possible options towards implementing the new fee component, namely \textit{prepaid outputs}, \textit{postpaid outputs} and \textit{scheduled payments}. We provide an analysis of the model with respect to all the three options. We show that state state growth could be bounded by a fee factor, miners are getting additional rewards, and lost coins are taken back into circulation eventually.    
\end{abstract}
...
\end{equation}
\dnote{fee size is not enforced here (in both regular script and $Height > self.height + H_{max}$)}
...
\dnote{With this script output is kept for free after $D_s$, if no one spent it.}
...
which is equals to 2 ($Mb \cdot Block$) for free for each ouptut.

This is exactly the increasing of upper bound for \state{} size according to equation \ref{eq:statesizemax}, because subsidized space-time will just prolongate outputs lifetime for corresponding value. However adversary strategy will change with $S_s$ - to maximize attack damage, she will maximize number of created outouts, getting $S_s$ space-time for each for free. In such a case she will be able to fill all blocks during subsidized period $S_s/B$ leading to additional 51 Gb of \state{} size upper bound in case of Bitcoin parameters.

Thus, additional subsidized space-time parameter allows to use the same blockchain for both money and data applications. Subsidized value may be configured to be big enough for regular users, allowing them to keep their outputs for free for some time, but small enough for blockchain usage as database. Subsidized period will increase \state{} size upper bound for 27\% in realistic conditions, while it allows to increase $K$ value painlessly for regular users, but increasing lost coin recirculation rate.
...
\begin{itemize}
  \item{}it is interesting to see how tokens in a cryptocurrency with a \esse{} fee component can be compared with a Gesell's demurrage currency~\knote{links}. 
